...
const update = () => {
  ...
  for (i = 0; i < conversation.length; i++){
    ...
    message.className =  conversation[i].side;
    message.innerHTML = `<img src="${conversation[i].name.toLowerCase()}.png"><p>${conversation[i].text}</p>`;
...
const post = () => {
  const textField = document.querySelector('.myMsg');
  const myMsg = textField.value;
  let message = document.createElement('div');
  message.className = 'local';
  message.innerHTML = `<img src="cleverbot.png"><p>${myMsg}</p>`;
  page.appendChild(message);
